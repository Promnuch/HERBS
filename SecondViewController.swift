...
class SecondViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate{
    ...
    override func viewDidLoad() {
        ...
        setupIdentifierConfidanceLabel()
        
    }
    
    fileprivate func setupIdentifierConfidanceLabel(){
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        guard let model = try? VNCoreMLModel(for: Herbs8().model) else { return}
        ...
        let request = VNCoreMLRequest(model: model) {(finishedReq, err) in
            ...
            DispatchQueue.main.async {
                ...
                // self.NameConfidance.text = "\(firstObservation.identifier) \(firstObservation.confidence * 100)"
            }
            // self.identifierLabel.text = ("Sorry I don't know")
        }
